...
// MARK: - Fetch Image
    func getMetaDataImage(urlString: String, completion: @escaping (LPLinkView) -> Void ) {
        guard let url = URL(string: urlString) else { return }
        let provider = LPMetadataProvider()
...
//    
//    func getMetaDataOnlyIcon(urlString: String, completion: @escaping (UIImage) -> Void ) {
//        guard let url = URL(string: urlString) else { return }
//        let cacheKey = NSString(string: urlString)
//        if let cachedImage = ImageCacheManager.shared.object(forKey: cacheKey) {
//            DispatchQueue.main.async {
...
//            else { return }
//            // Use the metadata
//            let _ = metadata.iconProvider?.loadObject(ofClass: UIImage.self) { icon, error in
//                let icon = icon as! UIImage?
//                guard let icon = icon else { return }
//                ImageCacheManager.shared.setObject(icon, forKey: cacheKey)
//                DispatchQueue.main.async {
